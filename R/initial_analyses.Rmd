...
## 2) Visualizing evolutionary trajectories
...
The trajectory of Ara+5 remains fairly constant over time. This may be because Ara+5 is not accumulating mutations in the set of genes that passed false-discovery rate correction ($I$). We will be examining this in subsequent analysis.
...
spec.no0.clean <- spec.no0[(spec.no0[,1] > 0.05) | (spec.no0[,1] < -0.05),] 
spec.no0.clean <- spec.no0.clean[(spec.no0.clean[,2] > 0.05) | (spec.no0.clean[,2] < -0.05),] 
...
## 3) Quantifying the variation in evolutionary outcomes
...
**1) "Bottom-up"**: For this approach we will simulate genotypic spaces under different evolutionary scenarios (i.e., sign epistasis, historical contingency, etc.), which will then be coarse-grained into individual genes.

**2) "Top-down"**: Here the degree of parallel evolution is constrained by the genetic background dependence of the distribution of fitness effects. This form of epistasis has been referred to as "macroscopic epistasis" and has been used in a previous approach towards inferring the evolutionary dynamics of the LTEE (Good and Desai, 2014). This approach has the potential advantage of providing a general constraint on the set of evolutionary outcomes based on few pieces of information, since it does not include information about the background dependence of individual mutations. 
...
Shoemaker WR & Lennon JT. The contribution of dormancy to microbial evolution. Society for Molecular Biology and Evolution, Austin, Texas, USA (2017).
