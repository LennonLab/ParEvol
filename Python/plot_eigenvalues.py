...
def get_predicted_observed_occupancies_geometric(df_np_, df_genes_, mut_counts_dict_):
    ...
    for mut_counts_idx, mut_counts in enumerate(df_np_):

        #probability_mut = effective_gene_lengths_nonsynonymous[df_genes_[mut_counts_idx]] / effective_gene_lengths_nonsynonymous['nonsynonymous']

        probability_mut = mut_counts / N_array_

        probability_mut_conditional = ((1 - probability_mut) ** (N_array_ - 1) ) * probability_mut

        predicted_occupancies_.append(np.mean(probability_mut_conditional))
...
ax_eigen_subsample.set_ylim([-0.005, 0.5])

ax_eigen_subsample.errorbar(eig_power_pops, eig_power_mean, yerr=[eig_power_mean - eig_power_975, eig_power_025 - eig_power_mean], fmt="o", alpha=1, barsabove=True, marker=".", ls="None",mfc="k", mec="k", c="k", zorder=2)
...
ax_poisson_mae.set_ylabel("Mean absolute error, Poisson", fontsize=12)
ax_poisson_mae.set_ylim([-0.005, 0.15])


ax_poisson_mae.errorbar(mae_non_x, mae_mean_non_y, yerr=[mae_mean_non_y - mae_mean_non_y_975, mae_mean_non_y_025 - mae_mean_non_y], fmt="o", alpha=1, barsabove=True, marker=".", ls="None", mfc="k", mec="k", c="k", zorder=2)


ax_poisson_mae.legend(loc="upper right", fontsize=8)

fig.subplots_adjust(wspace=0.3, hspace=0.3)
fig.savefig("%s/figs/eigen_occupancy.png" % pt.get_path(), format="png", bbox_inches="tight", pad_inches=0.2, dpi=600)
plt.close()
...
ax_geom_occupancy = plt.subplot2grid((1, 2), (0, 0))
#ax_geom_mae = plt.subplot2grid((1, 2), (0, 1))
ax_compare_mre = plt.subplot2grid((1, 2), (0, 1))

ax_geom_occupancy.text(-0.1, 1.07, "a", fontsize=10, fontweight="bold", ha="center", va="center", transform=ax_geom_occupancy.transAxes)
ax_compare_mre.text(-0.1, 1.07, "b", fontsize=10, fontweight="bold", ha="center", va="center", transform=ax_compare_mre.transAxes)


ax_geom_occupancy.plot([0.00001, 1], [0.00001, 1], lw=2.5, ls="--", c="k", zorder=1, label='1:1 line')
ax_geom_occupancy.scatter(observed_occupancies_geometric_non, predicted_occupancies_geometric_non, c="dodgerblue", alpha=0.8, zorder=2,)  # , c='#87CEEB')
ax_geom_occupancy.set_xlim([0.00007, 1.1])
ax_geom_occupancy.set_ylim([0.00007, 1.1])
ax_geom_occupancy.scatter(0.00000001, 0.000000001, alpha=0.8, c="dodgerblue", label="Gene")  # , c='#87CEEB')

# ax_poisson_occupancy.set_title('Nonsynonymous mutations', fontsize=11, fontweight='bold' )
ax_geom_occupancy.set_xscale("log", base=10)
ax_geom_occupancy.set_yscale("log", base=10)
ax_geom_occupancy.set_xlabel("Observed occupancy", fontsize=12)
ax_geom_occupancy.set_ylabel("Predicted occupancy, geometric", fontsize=12)
ax_geom_occupancy.legend(loc="upper left", fontsize=8)


mad_all_geom = np.mean(np.absolute(observed_occupancies_geometric_non - predicted_occupancies_geometric_non))
ax_geom_occupancy.text(0.18, 0.82, r"$\mathrm{MAE}=$" + str(round(mad_all_geom, 3)), fontsize=10, ha="center", va="center", transform=ax_geom_occupancy.transAxes)
...
#mae_non_x = list(mae_dict_non.keys())
#mae_non_x.sort()
#mae_mean_non_y = [mae_dict_non[x]["mae_mean_geometric"] for x in mae_non_x]
#mae_mean_non_y_975 = [mae_dict_non[x]["mae_975_geometric"] for x in mae_non_x]
#mae_mean_non_y_025 = [mae_dict_non[x]["mae_025_geometric"] for x in mae_non_x]
...
#ax_geom_mae.scatter(mae_non_x, mae_mean_non_y, c="dodgerblue")
#ax_geom_mae.axhline(y=mad_all, ls="--", lw=2, color="k", label=r"$\mathrm{MRE}$" + " of all populations")
#ax_geom_mae.axhline(y=0, ls=":", lw=2, color="grey")
#ax_geom_mae.set_xlabel("Number of replicate populations", fontsize=12)
#ax_geom_mae.set_ylabel("Mean relative error, geometric", fontsize=12)
#ax_geom_mae.set_ylim([-0.005, 0.13])
#ax_geom_mae.errorbar(mae_non_x, mae_mean_non_y, yerr=[mae_mean_non_y - mae_mean_non_y_975, mae_mean_non_y_025 - mae_mean_non_y], fmt="o", alpha=1, barsabove=True, marker=".", ls="None", mfc="k", mec="k", c="k", zorder=2)
#ax_geom_mae.legend(loc="upper right", fontsize=8)



mre_all_genes_poisson = np.absolute(observed_occupancies_non - predicted_occupancies_non) / observed_occupancies_non
mre_all_genes_geom = np.absolute(observed_occupancies_geometric_non - predicted_occupancies_geometric_non) / observed_occupancies_geometric_non
print("fraction of genes where gamma succeeds")
print(sum((mre_all_genes_poisson -mre_all_genes_geom) > 0) / len(mre_all_genes_geom)  )

ax_compare_mre.scatter(mre_all_genes_poisson, mre_all_genes_geom, c="dodgerblue", alpha=0.8, zorder=2)

ax_compare_mre.scatter(-10,-10, alpha=0.8, c="dodgerblue", label="Gene")
ax_compare_mre.plot([0.00001, 100], [0.00001, 100], lw=2.5, ls="--", c="k", zorder=1, label='1:1 line')
ax_compare_mre.set_xlim([0, 2])
ax_compare_mre.set_ylim([0.85, 1.05])
#ax_compare_mre.set_xscale("log", base=10)
#ax_compare_mre.set_yscale("log", base=10)
ax_compare_mre.set_xlabel("Relative error, Poisson", fontsize=12)
ax_compare_mre.set_ylabel("Relative error, geometric", fontsize=12)
ax_compare_mre.legend(loc="lower right", fontsize=8)


prop = dict(arrowstyle="-|>, head_width=0.4,head_length=1",
            shrinkA=0,shrinkB=0, facecolor='k', lw=3)


# xy = end
# xytext = start
ax_compare_mre.annotate("", xy=(1.9, 1.025), xytext=(1.2, 1.025), arrowprops=prop)
ax_compare_mre.annotate("", xy=(0.1, 1.025), xytext=(0.8, 1.025), arrowprops=prop)

ax_compare_mre.text(0.75, 0.93, "Geometric wins", fontsize=10, fontweight="bold", ha="center", va="center", transform=ax_compare_mre.transAxes)

ax_compare_mre.text(0.22, 0.93, "Poisson wins", fontsize=10, fontweight="bold", ha="center", va="center", transform=ax_compare_mre.transAxes)
...
fig.savefig("%s/figs/geom_occupancy.png" % pt.get_path(), format="png", bbox_inches="tight", pad_inches=0.2, dpi=600)
plt.close()
